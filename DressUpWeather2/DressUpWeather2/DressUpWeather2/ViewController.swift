...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func fetchData() -> Data? {
        guard let url = URL(string: "http://api.openweathermap.org/data/2.5/weather?lat=\(currentLocation.coordinate.latitude)&lon=\(currentLocation.coordinate.longitude)&appid=e4aefc24741504690052445e9b81243d") else { return nil }
    ...
    func parseJSON(from data: Data) {
        ...
        if let jsonWeatherData = try? jsonDecoder.decode(WeatherDataModel.self, from: data) {
            weatherData = jsonWeatherData
        }
        
        let temp = Int((weatherData?.main.temp ?? 0.0) - 273)
        let description = weatherData?.weather[0].main ?? "unknown"
        let detail = weatherData?.weather[0].description ?? "unknown"
        
        weatherDataLoaded(temp: temp, description: description, detail: detail)
    }
